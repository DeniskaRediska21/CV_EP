...
def detect_edges(image, l3, l4,h3, h4, kernel2,conv2_prev,ED_trashold,edge_detection_edge_number_trashold):
    ...
    # горизонтальная свертка изображения с kernrl2, берется только часть изображения выше нижнего конца горизонта
    ...
    # тут производится усреднение с прошлым conv2, чтобы избежать мерцания 
...
# Zeroing points bellow the horison line through cross product (обнуление полученных точек свертки ниже наклоненной линии горизонта)
    v1 = (l4 - l3, h4 - h3)
