...
def cluster_naive(points,cluster_treshold,clustering_cluster_number_trashold,cluster_centers_prev):
    ...
    while point_number < len(points):
        tmp_point = points[point_number,:] # текущи координаты точки
        points = points[(np.abs(points - points[point_number,:])>=distance_trashold).all(axis = 1)] # выкинуть все точки, которые ближе к текущей точке, чем distance_trashold
        points = np.r_[[tmp_point],points] # добавить текущую точку назад в начало массива
        point_number += 1
