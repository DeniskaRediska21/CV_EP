...
def detect_horison_pyramid(image,L,L_out,H,kernel,convolve_list,slices,num,horison_angle, horison_height, horison_angle_trashold, horison_angle_delta_trashold, horison_height_trashold, horison_height_delta_trashold,scaler = 15):
    ...
    image_r_2 = cp.array(image_r_2)
    for i in range(0,np.size(convolve_list)-1):
        M[i] = M[i] +scaler*np.shape(kernel)[0]/2+ (np.argmax(np.abs(convolve(cp.mean(image_r_2[M[i]:M[i]+int(H/scaler),convolve_list[i]:convolve_list[i+1]],axis = 1),kernel,'valid'))).get())
