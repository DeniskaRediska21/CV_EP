...
scaler = 1 # Увеличить разрешение изображения в scaler раз (чтобы протестировать быстродействие на других разрешениях)
cluster_treshold = 20*scaler # максимальное расстояние от точки, до кластера, чтобы считаться одним кластером (Для метода DBSCAN)
num = 4 # Кол-во точек в ядре свертки поиска горизонта
num2 = 5 # Кол-во точек в ядре свертки для поиска точек интереса (Edge detection)
fps = 30
ED_trashold = 20 # Порог для отсечения недостаточно интенсивных границ
bandwidth = 10* scaler # Настройка DBSCAN

test_accuracy = False # Проводить ли сверку с разметкой


# Настройки порогов срабатывания ошибки работы алгоритма
horison_angle_trashold = 30 # град.
horison_height_trashold = 400 # пкс.
horison_angle_delta_trashold = 10 # град/кадр
horison_height_delta_trashold = 100 # пкс./кадр
edge_detection_edge_number_trashold = 1000 # шт.
clustering_cluster_number_trashold = 30 # шт.
...
write = False # Производить ли запись в файл
verbose = True # Выдавать ли видео на экран
...
image = image[:,:L]
mid = int(L/2)
kernel_column = np.concatenate((np.linspace(0, -1, num = num), np.linspace(1,0,num = num))) # ядро свертки для поиска горизонта
S = np.shape(image)
...
kernel2 = np.concatenate((np.linspace(0, -1, num = num2), np.linspace(1,0,num = num2))) # ядро свертки для ED
...
# Framing
        [rect_centers,mean] = frame(cluster_centers,r_H,r_L) # Данный этап скорее всего не нужен, при использовании cluster_naive, нужно протестировать (пока реализовывать не нужно)
